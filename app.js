...
// V3.3 demo examples: staff comment, complaint, driver payment and no-show outcomes.
...
// Always keep the built-in demo records available during testing, while preserving any
// user-created records with different IDs. Built-in demo IDs are refreshed to current dates.
...
function renderQuotes(){const el=$('quotesBody');if(!el)return;el.innerHTML=quotes.map(q=>`<tr><td><strong>${q.id}</strong></td><td>${q.date}</td><td><strong>${escapeHtml(q.passenger)}</strong><div class="subline">${escapeHtml(q.phone||'')}</div></td><td class="route-col">${escapeHtml(q.pickup)}<div class="subline">to ${escapeHtml(q.dropoff)}</div></td><td>${sourceCell(q)}</td><td>${money(q.fare)}</td><td><span class="quote-status ${q.status.toLowerCase()}">${q.status}</span></td><td><button class="btn btn-light tiny" onclick="convertQuote('${q.id}')">Convert to Booking</button></td></tr>`).join('')||'<tr><td colspan="8">No quotes yet.</td></tr>'}
