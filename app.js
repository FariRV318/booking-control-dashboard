...
const statusPalette={Assigned:['#fff4d7','#9b6600'],Dispatched:['#e6efff','#2358b8'],'On the Way':['#e8e5ff','#5b43b5'],POB:['#def5ec','#15744f'],Completed:['#dcf4e4','#16703e'],Cancelled:['#ffe3e5','#aa2933']};
...
bookings=bookings.map(b=>({...b,dashboardComment:b.dashboardComment||''}));
...
function upcomingRow(b){const v=vehicles.find(x=>x.id===b.vehicleId);return `<tr>
...
<td class="dispatch-col"><select class="table-select status-select" style="${statusStyle(b.dispatch)}" onchange="updateDispatch('${b.id}',this.value)">${['Assigned','Dispatched','On the Way','POB','Completed','Cancelled'].map(s=>`<option ${b.dispatch===s?'selected':''}>${s}</option>`).join('')}</select></td>
...
function renderBookings(){
 ...
 $('allBookingsBody').innerHTML=list.length?list.map(b=>{const v=vehicles.find(x=>x.id===b.vehicleId);return `<tr><td class="ref-col"><span class="ref-link" onclick="editBooking('${b.id}')">${escapeHtml(b.id)}</span>${b.externalRef?`<div class="subline">Ext: ${escapeHtml(b.externalRef)}</div>`:''}</td><td class="date-col"><strong>${b.date}</strong><div class="subline">${b.time}</div></td><td class="passenger-col"><strong>${escapeHtml(b.passenger)}</strong><div class="subline">${escapeHtml(b.phone)}</div></td><td class="route-col"><strong>${escapeHtml(b.pickup)}</strong><div class="subline">→ ${escapeHtml(b.dropoff)}</div></td><td class="source-col" style="background:${accountColor(b.sourceId)}">${sourceCell(b)}</td><td class="driver-col">${escapeHtml(driverName(b.driverId))}</td><td class="vehicle-col">${v?`${escapeHtml(v.make)}<div class="subline">${escapeHtml(v.reg)}</div>`:'Not Assigned'}</td><td class="dispatch-col"><span class="status-pill" style="${statusStyle(b.dispatch)}">${escapeHtml(b.dispatch)}</span></td><td class="fare-col"><div class="fare-stack"><strong>Total ${money(b.fare)}</strong><span>Driver ${money(b.driverFare||0)}</span></div></td><td class="comment-col"><input class="inline-comment" value="${escapeHtml(b.dashboardComment||'')}" placeholder="Staff comment..." onchange="saveDashboardComment('${b.id}',this.value)"></td><td><button class="btn btn-light tiny" onclick="editBooking('${b.id}')">Open</button></td></tr>`}).join(''):'<tr><td colspan="11" class="muted">No bookings found.</td></tr>';
}
