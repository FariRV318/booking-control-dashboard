...
// Demo records intentionally exercise operational states.
...
// During prototype/testing, always keep current-date demo records present.
// User-created records with different IDs are preserved.
function mergeDemo(saved,demo){
 const existing=Array.isArray(saved)?saved:[];
 const demoIds=new Set(demo.map(x=>x.id));
 return [...demo.map(x=>({...x})),...existing.filter(x=>x&&x.id&&!demoIds.has(x.id))];
}
let accounts=mergeDemo(load('tbc_accounts_v3',load('tbc_accounts_v2',null)),defaultAccounts);
let vehicles=mergeDemo(load('tbc_vehicles_v3',load('tbc_vehicles_v2',null)),defaultVehicles);
let drivers=mergeDemo(load('tbc_drivers_v3',load('tbc_drivers_v2',null)),defaultDrivers);
let bookings=mergeDemo(load('tbc_bookings_v3',load('tbc_bookings_v2',null)),defaultBookings);
...
const statusPalette={Assigned:['#fff4d7','#9b6600'],Dispatched:['#e6efff','#2358b8'],'On the Way':['#e8e5ff','#5b43b5'],POB:['#def5ec','#15744f'],Completed:['#dcf4e4','#16703e'],Cancelled:['#ffe3e5','#aa2933'],'Driver No Show':['#f6dce0','#8b1e2d'],'Pax No Show':['#fbe0e4','#a52336']};
...
let quotes=mergeDemo(load('tbc_quotes_v3',null),defaultQuotes);
let expenses=mergeDemo(load('tbc_expenses_v3',null),defaultExpenses);
...
function bookedRevenue(arr){return arr.filter(b=>!['Cancelled','Driver No Show','Pax No Show'].includes(b.dispatch)).reduce((s,b)=>s+Number(b.fare||0),0)}
...
function renderDashboard(){
 ...
 $('upcomingBody').innerHTML=list.length?list.map(upcomingRow).join(''):'<tr><td colspan="13" class="muted">No active journeys match these filters.</td></tr>';
